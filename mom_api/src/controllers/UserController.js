 ...
 *
 *Changes in version 1.6 (FOUNDERSHARE (AKA PMP) - GIFTING REVAMP)
 * - [PMP-278] Added logic to handle register with isSignedUpWithEmailGiftLink flag
 *
 * @author TCSASSEMBLER
 * @version 1.6
 */
        ...
        function (cb) {
            if (registration.linkedSocialNetwork) {
                ...
                SecurityService.getSocialNetworkProfile(registration.linkedSocialNetwork, registration.linkedSocialNetworkAccessToken, cb.wrap(function (profile) {
                    ...
                    user.isSignedUpWithEmailGiftLink = registration.isSignedUpWithEmailGiftLink
                    cb();
                }));
            } else {
                ...
                error = validate(
                    {registration: registration},
                    {
                        registration: {
                            ...
                            isSignedUpWithEmailGiftLink: "bool?"
                        }
                    });
                if (error) {
                    return cb(error);
                }
                _.extend(user, _.pick(registration, "firstName", "lastName", "email", "password",
                    "interestedOfferCategory", "subscribedToNews", "isSignedUpWithEmailGiftLink"));
                cb();
            }
        }, function (cb) {
            if (!user.email) {
            ...
            user.verifyEmailText = user.isSignedUpWithEmailGiftLink
                ? 'verified'
                : helper.randomString(Const.SAFE_RANDOM_LENGTH);
            user.verifyEmailExpirationDate = user.isSignedUpWithEmailGiftLink
